...
func (b *Backend) DoCall(
	...
	length := len(res.Ret)
	if length > int(b.rpcCfg.ReturnDataLimit) && b.rpcCfg.ReturnDataLimit != 0 {
		return nil, fmt.Errorf("call retuned result on length %d exceeding limit %d", length, b.rpcCfg.ReturnDataLimit)
	}
