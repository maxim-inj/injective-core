...
func NewPublicAPI(logger log.Logger, evmBackend backend.EVMBackend) *PublicAPI {
	api := &PublicAPI{
...
func (e *PublicAPI) GetBlockReceipts(blockNrOrHash rpctypes.BlockNumberOrHash) ([]map[string]any, error) {
...
func (e *PublicAPI) GetTransactionLogs(txHash common.Hash) ([]*ethtypes.Log, error) {
	...
	cometBlockResult, err := e.backend.TendermintBlockResultByNumber(&res.Height)
	if err != nil {
