...
func (q legacyQueryServer) L3DerivativeOrderBook(
	...
	valuesConverter := utils.NewChainValuesConverter(unwrappedContext, market)
...
func (q legacyQueryServer) QueryExchangeParams(
	...
	resp := &v1.QueryExchangeParamsResponse{
		Params: utils.NewV1ExchangeParamsFromV2(respV2.Params),
	}
...
func (q legacyQueryServer) AggregateVolume(
	...
	for _, volume := range respV2.AggregateVolumes {
		...
		v1Volume := utils.NewV1MarketVolumeFromV2(valuesConverter, *volume)
...
func (legacyQueryServer) convertAggregateAccountVolumeRecords(
	...
	v2AggregateVolumes []*v2.AggregateAccountVolumeRecord,
	...
	for _, volume := range v2AggregateVolumes {
		...
		for _, marketVolume := range volume.MarketVolumes {
			...
			valuesConverter := utils.NewChainValuesConverter(ctx, market)

			v1Volume := utils.NewV1MarketVolumeFromV2(valuesConverter, *marketVolume)
...
func (legacyQueryServer) convertAggregateMarketVolumeRecords(
	ctx sdk.Context, marketFinder *marketfinder.CachedMarketFinder, v2AggregateMarketVolumes []*v2.MarketVolume,
	...
	for _, volume := range v2AggregateMarketVolumes {
		...
		v1VolumeRecord := utils.NewV1VolumeRecordFromV2(valuesConverter, volume.Volume)
		v := &v1.MarketVolume{
...
func (q legacyQueryServer) AggregateMarketVolume(
	...
	v1VolumeRecord := utils.NewV1VolumeRecordFromV2(valuesConverter, respV2.Volume)
...
func (q legacyQueryServer) AggregateMarketVolumes(
	...
	for _, v := range respV2.Volumes {
		...
		v1Volume := utils.NewV1MarketVolumeFromV2(valuesConverter, *v)
		volumes = append(volumes, &v1Volume)
	}
...
func (q legacyQueryServer) SpotMarkets(
	...
	for _, market := range respV2.Markets {
		...
		v1Market := utils.NewV1SpotMarketFromV2(valuesConverter, *market)
...
func (q legacyQueryServer) SpotMarket(ctx context.Context, request *v1.QuerySpotMarketRequest) (*v1.QuerySpotMarketResponse, error) {
	...
	if respV2.Market != nil {
		valuesConverter := utils.NewChainValuesConverter(unwrappedContext, respV2.Market)

		v1Market := utils.NewV1SpotMarketFromV2(valuesConverter, *respV2.Market)
...
func (q legacyQueryServer) FullSpotMarkets(
	...
	for _, market := range respV2.Markets {
		valuesConverter := utils.NewChainValuesConverter(unwrappedContext, market.Market)

		v1FullMarket := utils.NewV1FullSpotMarketFromV2(valuesConverter, *market)
...
func (q legacyQueryServer) FullSpotMarket(
	...
	if respV2.Market != nil {
		valuesConverter := utils.NewChainValuesConverter(unwrappedContext, respV2.Market.Market)

		v1FullSpotMarket := utils.NewV1FullSpotMarketFromV2(valuesConverter, *respV2.Market)
...
func (q legacyQueryServer) TraderSpotOrders(
	...
	for _, order := range respV2.Orders {
		v1TrimmedOrder := utils.NewV1TrimmedSpotLimitOrderFromV2(valuesConverter, order)
...
func (q legacyQueryServer) TraderDerivativeOrders(
	...
	for _, order := range respV2.Orders {
		v1TrimmedOrder := utils.NewV1TrimmedDerivativeLimitOrderFromV2(valuesConverter, *order)
		resp.Orders = append(resp.Orders, &v1TrimmedOrder)
	}
...
func (q legacyQueryServer) DerivativeMarkets(
	...
	for _, market := range respV2.Markets {
		...
		v1FullMarket := utils.NewV1FullDerivativeMarketFromV2(valuesConverter, *market)
		resp.Markets = append(resp.Markets, &v1FullMarket)
	}
...
func (q legacyQueryServer) DerivativeMarket(
	...
	v1FullMarket := utils.NewV1FullDerivativeMarketFromV2(valuesConverter, *respV2.Market)
...
func (q legacyQueryServer) ExchangeModuleState(
	...
	resp.State.Params = utils.NewV1ExchangeParamsFromV2(respV2.State.Params)
...
func convertMarkets(
	...
	for _, market := range respV2.State.DerivativeMarkets {
		...
		v1DerivativeMarket := utils.NewV1DerivativeMarketFromV2(valuesConverter, *market)
	...
	for _, market := range respV2.State.BinaryOptionsMarkets {
		...
		v1Market := utils.NewV1BinaryOptionsMarketFromV2(valuesConverter, *market)
...
func convertSpotOrderbooks(
	...
	for _, orderBook := range respV2.State.SpotOrderbook {
		...
		for _, order := range orderBook.Orders {
			v1Order := utils.NewV1SpotLimitOrderFromV2(valuesConverter, *order)
...
func convertDerivativeOrderbooks(
	...
	for _, orderBook := range respV2.State.DerivativeOrderbook {
		...
		for _, order := range orderBook.Orders {
			v1DerivativeOrder := utils.NewV1DerivativeLimitOrderFromV2(valuesConverter, *order)
...
func convertPositions(
	...
	for _, position := range respV2.State.Positions {
		...
		v1DerivativePosition := utils.NewV1DerivativePositionFromV2(valuesConverter, position)
		resp.State.Positions = append(resp.State.Positions, v1DerivativePosition)
	}
}

func convertSubaccountTradeNonces(
	respV2 *v2.QueryModuleStateResponse,
	resp *v1.QueryModuleStateResponse,
) {
...
func convertExpiryFuturesMarketInfoState(
	...
	for _, infoState := range respV2.State.ExpiryFuturesMarketInfoState {
		...
		v1InfoState := utils.NewV1ExpiryFuturesMarketInfoStateFromV2(valuesConverter, infoState)
		resp.State.ExpiryFuturesMarketInfoState = append(resp.State.ExpiryFuturesMarketInfoState, v1InfoState)
	}
}

func convertPerpetualMarketInfo(
	respV2 *v2.QueryModuleStateResponse,
	resp *v1.QueryModuleStateResponse,
) {
...
func convertPerpetualMarketFundingState(
	...
	for _, state := range respV2.State.PerpetualMarketFundingState {
		...
		v1FundingState := utils.NewV1PerpetualMarketFundingStateFromV2(valuesConverter, state)
...
func convertDerivativeMarketSettlementScheduled(
	...
	for _, settlement := range respV2.State.DerivativeMarketSettlementScheduled {
		...
		v1SettlementInfo := utils.NewV1DerivativeMarketSettlementInfoFromV2(valuesConverter, settlement)
...
func convertTradingRewardCampaignAccountPoints(
	...
	for _, point := range points {
		p := &v1.TradingRewardCampaignAccountPoints{
			...
			Points: utils.ConditionalNotionalToChainFormat(ctx, point.Points, 6),
		}
	...
	for _, point := range pendingPoints {
		...
		for _, accountPoint := range point.AccountPoints {
			p.AccountPoints = append(p.AccountPoints, &v1.TradingRewardCampaignAccountPoints{
				...
				// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
				Points: utils.ConditionalNotionalToChainFormat(ctx, accountPoint.Points, 6),
			})
		}
...
func convertFeeDiscountSchedule(
	...
	if respV2.State.FeeDiscountSchedule != nil {
		...
		for _, info := range respV2.State.FeeDiscountSchedule.TierInfos {
			// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
			chainFormatVolume := utils.ConditionalNotionalToChainFormat(ctx, info.Volume, 6)
	...
	for _, account := range respV2.State.FeeDiscountBucketVolumeAccounts {
		...
		for _, volume := range account.AccountVolume {
			// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
			chainFormatVolume := utils.ConditionalNotionalToChainFormat(ctx, volume.Volume, 6)
...
func convertHistoricalTradeRecords(
	...
	for _, record := range respV2.State.HistoricalTradeRecords {
		...
		v1TradeRecords := utils.NewV1TradeRecordsFromV2(valuesConverter, *record)
...
func convertLimitOrders(valuesConverter utils.ChainValuesConverter, v2Orders []*v2.DerivativeLimitOrder) []*v1.DerivativeLimitOrder {
	v1Orders := make([]*v1.DerivativeLimitOrder, 0, len(v2Orders))
	for _, order := range v2Orders {
		v1Order := utils.NewV1DerivativeLimitOrderFromV2(valuesConverter, *order)
		v1Orders = append(v1Orders, &v1Order)
	}
	return v1Orders
}

func convertMarketOrders(valuesConverter utils.ChainValuesConverter, v2Orders []*v2.DerivativeMarketOrder) []*v1.DerivativeMarketOrder {
	v1Orders := make([]*v1.DerivativeMarketOrder, 0, len(v2Orders))
	for _, order := range v2Orders {
		v1Order := utils.NewV1DerivativeMarketOrderFromV2(valuesConverter, *order)
		v1Orders = append(v1Orders, &v1Order)
	}
	return v1Orders
}
...
func (q legacyQueryServer) SubaccountPositionInMarket(
	...
	if respV2.State != nil {
		v1Position := utils.NewV1PositionFromV2(valuesConverter, *respV2.State)
...
func (q legacyQueryServer) PerpetualMarketInfo(
	...
	v1MarketInfo := utils.NewV1PerpetualMarketInfoFromV2(respV2.Info)
...
func (q legacyQueryServer) ExpiryFuturesMarketInfo(
	...
	v1ExpiryFuturesMarketInfo := utils.NewV1ExpiryFuturesMarketInfoFromV2(valuesConverter, respV2.Info)
...
func (q legacyQueryServer) PerpetualMarketFunding(
	...
	resp := &v1.QueryPerpetualMarketFundingResponse{State: utils.NewV1PerpetualMarketFundingFromV2(valuesConverter, respV2.State)}
...
func (q legacyQueryServer) TradeRewardPoints(
	...
	for _, points := range respV2.AccountTradeRewardPoints {
		resp.AccountTradeRewardPoints = append(
			...
			utils.ConditionalNotionalToChainFormat(unwrappedContext, points, 6),
		)
	}
...
func (q legacyQueryServer) TradeRewardCampaign(
	...
	resp := &v1.QueryTradeRewardCampaignResponse{
		// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
		TotalTradeRewardPoints: utils.ConditionalNotionalToChainFormat(
			unwrappedContext,
	...
	if respV2.TradingRewardCampaignInfo != nil {
		v1CampaignInfo := utils.NewV1TradingRewardCampaignInfoFromV2(respV2.TradingRewardCampaignInfo)
	...
	for _, totalRewardPoints := range respV2.PendingTotalTradeRewardPoints {
		// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
		chainFormattedPoints := utils.ConditionalNotionalToChainFormat(unwrappedContext, totalRewardPoints, 6)
...
func (q legacyQueryServer) FeeDiscountAccountInfo(
	...
	if respV2.AccountInfo != nil {
		resp.AccountInfo = &v1.FeeDiscountTierInfo{
			...
			// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
			Volume: utils.ConditionalNotionalToChainFormat(unwrappedContext, respV2.AccountInfo.Volume, 6),
		}
	}
...
func (q legacyQueryServer) FeeDiscountSchedule(
	...
	for _, info := range respV2.FeeDiscountSchedule.TierInfos {
		resp.FeeDiscountSchedule.TierInfos = append(resp.FeeDiscountSchedule.TierInfos, &v1.FeeDiscountTierInfo{
			...
			StakedAmount:      info.StakedAmount,
			// Historically all tokens with FeeDiscount had 6 decimals (until the migration to exchange v2 - v1.16.0)
			Volume: utils.ConditionalNotionalToChainFormat(unwrappedContext, info.Volume, 6),
		})
	}
...
func (q legacyQueryServer) MarketVolatility(
	...
	for _, record := range respV2.RawHistory {
		v1TradeRecord := utils.NewV1TradeRecordFromV2(valuesConverter, *record)
...
func (q legacyQueryServer) DenomMinNotional(
	...
	res := &v1.QueryDenomMinNotionalResponse{
		Amount: utils.ConditionalNotionalToChainFormat(ctx, respV2.Amount, denomDecimals),
	}
...
func (q legacyQueryServer) DenomMinNotionals(
	...
	for _, v2DenomMinNotional := range respV2.DenomMinNotionals {
		...
		v1DenomMinNotionals = append(v1DenomMinNotionals, &v1.DenomMinNotional{
			...
			MinNotional: utils.ConditionalNotionalToChainFormat(
				unwrappedContext,
