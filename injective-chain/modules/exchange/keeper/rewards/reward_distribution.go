...
func (k TradingKeeper) ProcessTradingRewards(ctx sdk.Context) {
	...
	shouldStartNextCampaign := rewardPool != nil &&
		campaignInfo != nil &&
		rewardPool.StartTimestamp != 0 &&
		blockTime >= rewardPool.StartTimestamp
...
func (k TradingKeeper) DistributeTradingRewards(
	ctx sdk.Context,
...
func (k TradingKeeper) distributeTradingRewardsForAccount(
	ctx sdk.Context,
	...
	for _, coin := range maxCampaignRewards {
		...
		if coin.Denom == chaintypes.InjectiveCoin && accountRewardAmount.GT(injRewardStakedRequirementThreshold) {
			...
			stakedINJ := k.feeDiscounts.CalculateStakedAmountWithoutCache(ctx, accountPoints.Account, maxDelegations)
...
func (k TradingKeeper) distributeTradingRewardsForAllAccounts(
	ctx sdk.Context,
	availableRewardsToPayout map[string]math.Int,
	maxCampaignRewards sdk.Coins,
	pendingPoolStartTimestamp int64,
	...
	events.Emit(ctx, k.BaseKeeper, &ev)
	k.DeleteTotalTradingRewardPendingPoints(ctx, pendingPoolStartTimestamp)
}

func (k TradingKeeper) getAvailableRewardsToPayout(
	ctx sdk.Context,
	maxCampaignRewards sdk.Coins,
) map[string]math.Int {
	ctx, doneFn := metrics.ReportFuncCallAndTimingSdkCtx(ctx, k.svcTags)
